{ pkgs, config, herdr, ... }:
{
  ...
  # directly in this repo (commit or discard them as you like). scripts/theme-render
  # owns the marker-delimited [theme] block in that file; everything else is
  # hand-maintained. Assumes the repo is checked out at ~/dotfiles-nixos; adjust
  # the subpath if a host clones it elsewhere.
